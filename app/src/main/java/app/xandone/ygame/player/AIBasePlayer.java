...
/**
 * author: Admin
 * created on: 2018/12/11 13:49
 * description:
 */
public class AIBasePlayer extends Sprite {

    public AIBasePlayer(Bitmap bitmap, int w, int h, List<Point> freePoints) {
    ...
    private void initRange(List<Point> comuters, List<Point> humans) {
        ...
        for (Point point : humans) {
            if (point.x - RANGE_STEP < currentRange.xStart) {
                ...
                currentRange.xStop = point.y + RANGE_STEP;
            }
        ...
        currentRange.xStop = currentRange.xStop >= maxW ? maxW - 1 : currentRange.xStop;
        currentRange.yStop = currentRange.yStop >= maxH ? maxH - 1 : currentRange.yStop;
    }
    ...
    private Point getFirstPoint(List<Point> humans) {
        ...
        if (point.x == 0 || point.y == 0 || point.x == maxW && point.y == maxH)
            return new Point(maxW / 2, maxH / 2);
        else {
...
//			}
    ...
    private boolean isOutSideOfWall(Point point, int direction) {
        if (direction == HENG) {
            return point.x < 0 || point.x >= maxW;//最大的X和Y值均在墙外所以用等号
        } else if (direction == ZHONG) {
            return point.y < 0 || point.y >= maxW;
        } else {//这里可能有问题
            return point.x < 0 || point.y < 0 || point.x >= maxW || point.y >= maxH;
        }
    }
        ...
        switch (direction) {
            ...
            case HENG:
                while (!enemyPoints.contains(point) && point.x >= 0 && count <= 6) {
                    count++;
                    point.set(point.x - 1, y);
                }
                point.set(x, y);
                while (!enemyPoints.contains(point) && point.x < maxH && count <= 6) {
                    count++;
                    point.set(point.x + 1, y);
                }
                break;
            ...
            case ZHONG:
                while (!enemyPoints.contains(point) && point.y > 0) {
                    count++;
                    point.set(x, point.y - 1);
                }
                point.set(x, y);
                while (!enemyPoints.contains(point) && point.y < maxH && count <= 6) {
                    count++;
                    point.set(x, point.y + 1);
                }
                break;
            //正斜向 /
            case ZHENG_XIE:
                while (!enemyPoints.contains(point) && point.x >= 0 && point.y < maxH) {
                    count++;
                    point.set(point.x - 1, point.y + 1);
                }
                point.set(x, y);
                while (!enemyPoints.contains(point) && point.x < maxW && point.y >= 0 && count < 7) {
                    count++;
                    point.set(point.x + 1, point.y - 1);
                }
                break;
            ...
            case FAN_XIE:
                ...
                point.set(x, y);
                while (!enemyPoints.contains(point) && point.x < maxW && point.y < maxH && count < 6) {
                    count++;
                    point.set(point.x + 1, point.y + 1);
                }
                break;
